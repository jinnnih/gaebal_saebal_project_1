...
> 이 그림은 손으로 그리지 않는다. `python3 tools/render_layout.py` 가
> `config/parking_spots.json` 과 `maps/parking_lot.pgm` 만 읽어 다시 그리므로,
> 기하를 바꾼 뒤에는 생성기와 함께 이것도 돌려야 그림이 실제와 맞는다.

*파랑=일반 주차면, 초록=장애인, 노랑=EV 충전, 빨강=초기 점유, 빗금=안전지대(주차 불가).
파란 파선 = 통로 중심선(옆에 폭 표기), 회색 배경 = 정적 맵(벽·기둥·게이트 개구부).
초록 점·사각 = 입구 포즈와 그 자리의 차체 크기, 빨강 점 = 출구 포즈.
주차 진입/출차 방향은 면마다 다르므로 `config/parking_spots.json` 의
`entry_side` / `goal_pose` / `prepark_pose` 를 볼 것.*

## 1. 주차장 사양

| 항목 | 값 |
|---|---|
| 내부 크기 | 56.00 m (X) × 48.10 m (Y) |
| 주차면 | 56 면 — 주차가능 **54**, 빗금 안전지대 2 |
| 초기 점유 | 22 면 (결정론적 배치 → 데모 재현성) |
| 주차면 규격 | 2.50 m × 5.40 m |
| 통로 | 남/북 외곽 8.00 m, 중앙 **10.50 m**, 동/서 연결차로 10.00 m |
| 구조 기둥 | 0.5 m 각, 16 개 (A/B, C/D 열 경계선 위, 2면마다) |
| 입구 | 서벽 개구부 5.0 m, 중심 y = −20.05, 진입 자세 yaw **45°** |
| 출구 | 동벽 개구부 5.0 m, 중심 y = +20.05 |
| 맵 그리드 | 1160 × 1002 px @ 0.05 m/px, origin (−29.00, −25.05) |

### 열(Row) 배치 — 남에서 북으로

```
  y=+24.05 ┌──────────────── 북 벽 ─────────────────┐ ── 출구(동벽 y=+20.05)
           │        Aisle_N  (8.0 m)                │
  y=+16.05 ├────────────────────────────────────────┤
  ...
  y=-24.05 └──────────────── 남 벽 ─────────────────┘
 입구(서벽 y=-20.05)
       x=-28.00        주차면 블록 x∈[-17.5,+17.5]        x=+28.00
                       (블록 x 는 통로를 넓혀도 바뀌지 않는다)
```
...
- `prepark_pose` 에서 차체가 통로 밖으로 나가지 않음 (남/북 통로 8.0 m 에 4.5 m 차체 + 여유 3.5 m)
- `goal_pose` 풋프린트 9점이 전부 정적 맵상 free 셀

> **통로 폭을 다시 손대야 한다면** `tools/generate_parking_lot.py` 상단의
> `AISLE_SIDE` / `AISLE_CENTER` / `LANE_W` 만 바꾸고 재생성할 것. 한 줄만 바꾸면 월드·맵·
> 주차면 테이블·Nav2 파라미터가 전부 일관되게 다시 나온다.
>
> 2026-09-02~03 에 두 번 넓혔다 (7.0/8.0/7.0 → 8.0/9.0/10.0). 이유는 생성기 상수
> 주석에 적어 뒀다. 요약하면, 기존 연결차로는 순폭 7.30 m 라 `inflation_radius`(2.60)를
> 빼면 무비용 밴드가 2.10 m 뿐이었고, MPPI 추종 오차(0.5~1.6 m)가 이를 넘으면
> 차체가 keepout 에 걸쳐 플래너가 `"Start occupied"` 로 재계획을 거부했다.
> 넓힌 뒤 무비용 밴드는 4.10 m 다.
>
> **입구 진입 자세는 yaw 45° 다.** 통로 폭과 별개의 제약이다. yaw 0 으로 두면
> 입구에서 서측 차로에 붙기까지 90° 를 돌아야 하는데, 그 회전 도중 앞바깥
> 모서리가 A 행 쪽으로 크게 쓸고 지나간다. 거기에 추종 오차 0.7 m 가 얹히면
> 모서리가 keepout 인플레이션의 253 대역에 닿고, MPPI 가 전진을 거부해
> 후진/전진 셔플에 갇힌다. 45° 로 두면 필요한 회전이 절반이라 포락선 최동단이
> −19.53 → −21.69 로, 블록까지 여유가 1.73 → **3.89 m** 가 된다.
> 생성기의 `check_entry_turn()` 이 이 조건을 매 생성마다 검사한다.
...
#   시작 포즈: x=-24.00  y=-20.05  yaw=45deg  (게이트 통과하며 좌회전 시작)
