...
# ! 9.00 -> 10.50. 차로에서 통로로 90 도 붙을 때의 여유 때문이다.
#   9.00 일 때 그 여유가 0.06 m 뿐인데 실측 추종 오차가 0.70 m 다.
#   대칭 확대라 주차행 북단이 확대량의 절반만큼 남하한다.
#     +1.0 -> 여유 0.56 (부족)
#     +1.5 -> 여유 0.81 (실측 추종 오차 0.70 초과)
#   같은 효과를 keepout 진입 여유(KEEPOUT_APRON)로 내려던 시도는 역효과였다.
#   그건 주차면 안쪽을 비우는 것이라 경로가 주차행 안으로 들어가 버린다.
#   이건 주차행 자체를 벌리는 것이라 통로가 실제로 넓어진다.
#   대신 y 좌표가 전부 이동한다 (x 는 불변).
AISLE_CENTER = S(10.50)  # 중앙 통로 폭 (B/C 열 공용, 회전 여유)
...
planner_server:
  ros__parameters:
    ...
    GridBased:
      ...
      allow_unknown: false
      # ! 1.2M / 5.0 s 로 되돌렸다가 되물렸다. 이유는 실측이다.
      #   입구->출구 75 m 대각선이 어떤 회차엔 0.4 s 에 풀리는데 어떤
      #   회차엔 "exceeded maximum iterations" 로 실패한다. 경계값이었다.
      #   한 번 실패하면 BT 가 BackUp 복구를 돌려 차가 게이트 쪽으로 밀리고,
      #   거기서 또 실패해 빠져나오지 못한다 (3회 연속 출발점에서 실패).
      #   실패까지 5~9 s 걸리는 걸 보면 1.2M 회에 4~5 s 라, max_planning_time
      #   5.0 과 거의 같은 지점에서 잘린다. 둘 다 올려야 의미가 있다.
      ...
      # 통로 중앙 선호도. inflation_radius 를 외접원 이상으로 키워
      # 통로에 비용 기울기가 생겼으므로 이 값이 실제로 작동한다.
      # ! 4.0 -> 7.0. 4.0 으로 되돌려 봤더니 확실히 나빴다 (실측):
      #     입구->출구   2/2  ->  0/3
      #     Optimizer 실패  1  ->  94 회
