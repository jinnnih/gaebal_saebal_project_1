...
| 형상 | **실차 메시** (Prius Hybrid, CC0) 를 제원에 맞춰 변형 | `meshes/README.md` |
| 총 질량 | 913 kg | — |
| 구동 방식 | 후륜 구동 + 전륜 조향 | `ackermann_steering_controller` |

외형은 **실차 메시**다. Gazebo Fuel 의 Prius Hybrid(CC0)를 우리 제원에 맞춰 변형해서 쓴다.

* **visual = 메시, collision = 박스 6개.** 실제 로봇 description 의 표준 방식이다.
  물리는 박스로 빠르고 안정적으로 풀고, 눈에 보이는 것만 메시로 한다.
* 원본 Prius 는 축거 2.865 / 전장 4.618 이라 그냥 스케일하면 바퀴가 휠아치에서
  15 cm 어긋난다. `tools/fit_car_mesh.py` 가 길이축을 **구간별 선형 리매핑** 해서
  전장 4.50 / 축거 2.50 을 정확히 맞춘다. 자세한 건 `meshes/README.md`.
* `mesh:=false` 로 띄우면 박스 조합 외형으로 돌아간다 (메시 없이 확인할 때).

메시를 넣어도 **성능 손해가 없다** — RTF 1.002, 라이다 100% 유효로 박스 시절과 동일하다
(1680 면밖에 안 된다).

라이다는 지붕 위 1.42 m — 이유는 아래 참고.
...
```bash
ros2 launch valet_robot valet_sim.launch.py x:=-24.00 y:=-19.30 yaw:=0.7854
```
...
`tools/smoke_probe.py` 를 실제 시뮬에서 돌린 결과다. **8/8 통과.**

| 항목 | 결과 |
|---|---|
| B. `odom → base_footprint` TF | 정상 |
| C. 전진 8 s @1.0 m/s | 이동 **8.00 m**, 방향변화 0.6° |
| D. 최소회전반경 선회 | 실측 **R = 3.47 m** (요구 3.5704 이하 — 만족) |
| E. Ackermann 내륜차 | 좌 **40.18°** / 우 **29.95°** (차 10.23°) — 좌회전에서 좌측이 내륜 ✅ |
| F. 후진 5 s @-0.5 m/s | 차체 전방 기준 **-2.50 m** |
| G. 제자리회전 차단 | 이동 0.000 m, 회전 0.00° |
| H. 각속도 클램프 | 요청 2.0 rad/s → 실측 **0.254**, 상한 0.280 |
| A. `/scan` | **40 프레임 중 유효 40 (100%)**, 평균 427/720 빔, 최대 **29.5 m**, frame=`lidar_link` |
| — | 아래는 실차 메시 적용 후 재측정값 (성능 동일) |
...
### (1) 라이다 렌더링 — **GLX + llvmpipe 가 정답이다**

가장 오래 막혔던 문제이고, 중간에 한 번 반대로 결론냈다가 되잡았다.

같은 월드·로봇으로 40 프레임씩 재본 값이다.

| 렌더 경로 | 유효 프레임 | 평균 빔 | 최대 관측거리 | RTF |
|---|---|---|---|---|
| EGL + SVGA3D 하드웨어 (기본 헤드리스) | **8 %** | 173/720 | 4.9 m | 1.00 |
| GLX + SVGA3D 하드웨어 (`DISPLAY` 지정) | 15 % | 125/720 | 29.5 m | 1.00 |
| **GLX + llvmpipe 소프트웨어** | **100 %** | **427/720** | **29.5 m** | **1.00** |

`gz sim -s` 를 그냥 띄우면 Ogre2 가 **EGL** 로 SVGA3D 하드웨어를 잡는데, 이 경로의
오프스크린 깊이 렌더링이 망가져 있다. 프레임의 90% 이상이 통째로 비거나 일부만 채워지고,
관측거리도 5 m 근처에서 잘린다.

여기서 `LIBGL_ALWAYS_SOFTWARE=1` 을 주면 mesa 가 거부한다.
...
그래서 한동안 "헤드리스에서 소프트웨어 렌더링을 쓰면 안 된다" 고 결론냈는데, **틀렸다.**
거부당하는 건 EGL 경로뿐이다. **X 디스플레이를 물려 GLX 로 가면** llvmpipe 가 정상
...
### (2) GUI 는 프로세스를 나눠 붙인다 — 깜빡임도 여기서 해결된다

서버(`gz sim -s`)와 GUI(`gz sim -g`)는 **별개 프로세스**다. 물리와 센서 렌더링은
...
| GUI 렌더링 | 깜빡임 | 서버 RTF | 라이다 |
|---|---|---|---|
| 하드웨어 GL (`HW=1`) | **있음** | 1.000 | 100 %, 642 빔 |
| **llvmpipe (기본)** | **없음** | **1.088** | **100 %, 651 빔** |

즉 GUI 를 소프트웨어로 돌리는 데 따르는 손해가 사실상 없다.

### (3) gz-sim 이 gpu_lidar 를 하나만 렌더링한다
...
"월드 로드 시점에 센서를 하나 둬서 렌더 씬을 워밍업한다" 는 대응을 넣었다가 오히려
로봇 라이다를 죽이는 것을 확인하고 되돌렸다.
