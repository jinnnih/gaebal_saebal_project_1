...
| D. 최소회전반경 선회 | 실측 **R = 3.46 m** (요구 3.5704 이하 — 만족) |
| E. Ackermann 내륜차 | 좌 **40.13°** / 우 **32.32°** (차 7.81°) — 좌회전에서 좌측이 내륜 ✅ |
...
### (2) GUI 와 라이다를 같이 쓰려면 프로세스를 나눈다

서버까지 GUI 와 같은 환경으로 띄우면 1228x875 창을 llvmpipe 로 그리게 돼 느리다.
GUI 는 **별도 프로세스**라 하드웨어 GL 로 따로 붙일 수 있다.

```bash
터미널 1:  bash run_sim.sh      # 서버 + 라이다 (llvmpipe)
터미널 2:  bash show_gui.sh     # GUI 창만 (하드웨어 GL)
```

실측: GUI 를 켠 상태에서 **라이다 100% 유효, 642/720 빔, 최대 29.8 m, RTF 1.000**.
