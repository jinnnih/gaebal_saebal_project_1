...
| 전고 | 0.94 m (라이다 포함) | 아래 "설계 결정 2" |
| 총 질량 | 913 kg | — |
| 구동 방식 | 후륜 구동 + 전륜 조향 | `ackermann_steering_controller` |

차체는 **저상형 자율주행 플랫폼** 형태다(승용차 형상이 아님). 그래야 라이다를
주차 차량 차체 높이에 두면서도 자기 몸에 가리지 않는다 — 아래 참고.
...
### 2) 라이다를 **0.92 m** 에 뒀다 (지붕이 아니라)

주차장 월드의 장애물 높이를 역산한 결과다.

```
 parked_car 차체   0.27 ~ 0.99 m   <- 여기를 때려야 빈 주차면 탐색이 성립
 parked_car 캐빈   0.99 ~ 1.51 m
 벽 / 기둥         0.00 ~ 2.60 m
 EV 충전기         0.00 ~ 1.30 m
 게이트            0.00 ~ 1.20 m
 라바콘            0.00 ~ 0.59 m   <- 정적 맵(parking_lot.pgm)에 이미 포함
 ------------------------------------------------------
 자차 데크 상단              0.82 m
 라이다                      0.92 m   <- 자기 몸은 넘고 주차 차량은 때린다
```

라이다를 승용차처럼 지붕(1.6 m)에 올리면 주차 차량 위를 **넘어가서**
`obstacle_layer` 가 주차 차량을 못 본다. 그러면 "빈 주차면을 찾아서 주차한다"는
데모의 전제가 무너진다. 반대로 0.82 m 아래로 내리면 자기 데크에 가린다.
이 두 제약 때문에 차체를 저상형으로 설계했다.
...
| 항목 | 결과 |
|---|---|
| A. `/scan` | 유효 프레임 32%, 유효 시 평균 213/720 빔, 최대 **29.9 m**, frame=`lidar_link` |
| B. `odom → base_footprint` TF | 정상 |
| C. 전진 8 s @1.0 m/s | 이동 **7.99 m**, 방향변화 0.0° |
| D. 최소회전반경 선회 | 실측 **R = 4.02 m** (기하 3.3~3.5 m, 과도구간·슬립 포함) |
| E. Ackermann 내륜차 | 좌 **32.24°** / 우 **41.98°** (차 9.73°) — 포화 없음 |
| F. 후진 5 s @-0.5 m/s | 차체 전방 기준 **-2.50 m** |
| G. 제자리회전 차단 | 이동 0.000 m, 회전 0.00° (차량형이므로 정상) |
| H. 각속도 클램프 | 요청 2.0 rad/s → 실측 0.131, 상한 0.280 |
...
> **D 의 4.02 m 에 대해** — 정지 상태에서 출발해 14 s 선회한 값이라 가속·조향
> 램프업 구간이 섞여 있다. 실측 조향각으로 역산한 정상상태 기하 반경은
> 3.3~3.5 m 로 Nav2 의 3.5704 m 제약을 만족한다. 3주차 후진 주차 튜닝 때
> 정상상태에서 다시 측정할 것.
